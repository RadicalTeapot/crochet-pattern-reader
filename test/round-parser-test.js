...
import { RoundParser } from '../src/round-parser.js';

let it = testSuite('RoundParser');
...
it('Works when no previous round is set', () => {
    const parser = new RoundParser();
...
it('Fails when assigning count when no previous round is set', () => {
    const parser = new RoundParser();
    assert.throws(() => parser.parseRound({ type: 'round', stitches: [{ type: 'stitch', name: 'sc', count: 0 }] }));
});

it('Works when previous round is set', () => {
    const parser = new RoundParser(new Round([new Stitch('sc', 1, 1)]));
    const round = parser.parseRound({ type: 'round', stitches: [{ type: 'stitch', name: 'sc', count: 1 }] });
    assert.deepStrictEqual(round, new Round([new Stitch('sc', 1, 1)]));
});

it('Assigns correct count when previous round is set', () => {
...
it('Uses previous round when called multiple times', () => {
    const parser = new RoundParser();
    const round = parser.parseRound({ type: 'round', stitches: [{ type: 'stitch', name: 'sc', count: 1 }] });
    assert.deepStrictEqual(round, new Round([new Stitch('sc', 1, 1)]));
